...
API key — X-Api-Key: <SECA_API_KEY> header on protected endpoints
JWT session tokens — issued by /auth/* routes for player sessions (Authorization: Bearer <token>)
...
docker compose up
API at http://localhost:8000. Requires Ollama on the host:
...
python-tests — golden tests, contract tests, API contract validation, pipeline regression, explain schema validation, full CI suite, Black/Pylint/Mypy gates, coverage ≥ 80%, pip-audit, Trivy
android-build — Gradle build and host JVM test suite
docker-build — builds cyberchesscoach (Android) and cyberchesscoach-llm-api (backend) images
deploy — triggered on v*.*.* tag; publishes to GHCR and deploys to production
...
All CI-safe tests pass (golden, contract, API contract, pipeline regression)
LLM regression tests pass (test_llm_regression.py)
Real LLM smoke test passes (test_ollama_smoke.py)
Manual output sanity review (no engine mentions, no move suggestions, correct tone)
...
Correctness — Invariants enforced via code and test contracts, not convention
Determinism — All layers except LLM generation are fully reproducible
Non-hallucination — ESV normalization and output validators guarantee the LLM cannot invent engine facts
Safety — Strict contracts on all LLM outputs; no output is always better than unsafe output
Maintainability — Loose coupling with explicit trust boundaries at every layer
