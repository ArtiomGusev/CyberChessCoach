...
def test_fuzz_scores_monotonic_between_high_and_low_samples():
    ...
    assert mean(high_scores) >= MIN_QUALITY_SCORE, f"High-quality mean {mean(high_scores)} < MIN_QUALITY_SCORE"
    # Low-quality mean should be strictly less than high-quality mean by at least 1
    assert mean(high_scores) - mean(low_scores) >= 1.0, "Expected separation between high and low samples"
...
def test_fuzz_no_exceptions_for_random_texts():
    ...
    for _ in range(SAMPLES * 2):
        ...
        sc = score_explanation(text=text, engine_signal=engine_signal)
        assert 0 <= sc <= 10, f"Score out of bounds for random text: {sc}"
