...
def test_ci_workflow_hardens_checkout_and_supply_chain_controls():
    ...
    assert _step_named(image_security, "Scan published app image")["with"]["scan-type"] == "image"
    assert (
        _step_named(image_security, "Scan published llm API image")["with"]["scan-type"] == "image"
    )
